...
export class Post {
  ...
  constructor(raw?: PostRaw) {
    if (raw) {
      ...
      // Generate text from excerpt or content if text is empty
      if (!this.text && (this.excerpt || this.content)) {
        const source = this.excerpt || this.content || ''
        // Strip HTML tags and get plain text (works in browser environment)
        if (typeof document !== 'undefined') {
          const tempDiv = document.createElement('div')
          tempDiv.innerHTML = source
          this.text = tempDiv.textContent || tempDiv.innerText || ''
        } else {
          // Fallback for SSR: simple regex to remove HTML tags
          this.text = source.replace(/<[^>]*>/g, '').replace(/&[^;]+;/g, ' ').trim()
        }
        // Limit to 80 characters for excerpt to keep card compact
